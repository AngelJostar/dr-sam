...
(() => {
  ...
  const registerPatientName = <?php echo json_encode($patient->full_name, 15, 512) ?>;
  ...
  const aiChatStorageKey = <?php echo json_encode('drsam_patient_ai_chat_'.$patient->id, 15, 512) ?>;
  ...
  doctorSelectButtons.forEach(button => button.addEventListener('click', () => selectDoctorResult(button)));
