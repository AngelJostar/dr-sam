  ...
  <form class="superadmin-oncology-form" onsubmit="event.preventDefault()">
    ...
    <div class="superadmin-oncology-body">
      ...
      <div class="superadmin-oncology-table-wrap"><table class="superadmin-oncology-table">
        <thead><tr><th rowspan="2">No.</th><th rowspan="2">Medicamento*</th><th rowspan="2">Dosis*</th><th colspan="3">Diluyente*</th><th rowspan="2">Volumen de dilucion total ml*</th><th rowspan="2">No. de bolos x dia*</th><th rowspan="2">Tiempo de infusion (min)</th><th colspan="4">Via de administracion*</th><th colspan="6">Fecha de entrega*</th></tr><tr><th>CS</th><th>DX</th><th>Otro</th><th>IV</th><th>IM</th><th>SC</th><th>Otro</th><?php for($date = 1; $date <= 6; $date++): ?><th>Fecha <?php echo e($date); ?></th><?php endfor; ?></tr></thead>
        <tbody><?php for($row = 1; $row <= 8; $row++): ?><tr><th><?php echo e($row); ?></th><td><select aria-label="Medicamento renglon <?php echo e($row); ?>"><option value="">Seleccionar</option><?php $__currentLoopData = $oncologyMedicines; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $medicine): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option><?php echo e($medicine); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></select></td><td><input aria-label="Dosis renglon <?php echo e($row); ?>"></td><?php for($check = 1; $check <= 3; $check++): ?><td><input type="checkbox" aria-label="Diluyente <?php echo e($check); ?> renglon <?php echo e($row); ?>"></td><?php endfor; ?><td><input aria-label="Volumen renglon <?php echo e($row); ?>"></td><td><input aria-label="Bolos renglon <?php echo e($row); ?>"></td><td><input aria-label="Tiempo renglon <?php echo e($row); ?>"></td><?php for($check = 1; $check <= 4; $check++): ?><td><input type="checkbox" aria-label="Via <?php echo e($check); ?> renglon <?php echo e($row); ?>"></td><?php endfor; ?> <?php for($date = 1; $date <= 6; $date++): ?><td class="is-date"><input type="date" aria-label="Fecha <?php echo e($date); ?> renglon <?php echo e($row); ?>"></td><?php endfor; ?></tr><?php endfor; ?></tbody>
      </table></div>
  ...
  <form class="superadmin-npt-form" onsubmit="event.preventDefault()">
    ...
    <div class="superadmin-npt-body">
      ...
      <fieldset><legend>PRODUCTOS ACTIVOS DEL CATALOGO</legend><div class="superadmin-npt-products">
        <?php $__currentLoopData = $nptProducts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><label><?php echo e($product); ?><input type="number" min="0" step="0.01" name="npt_product_<?php echo e($index); ?>"></label><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php /**PATH C:\laragon\www\dr-sam-laravel\resources\views/superadmin/_mix_request_format.blade.php ENDPATH**/ ?>