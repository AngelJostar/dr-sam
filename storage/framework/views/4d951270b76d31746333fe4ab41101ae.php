...
<section
  ...
  data-initial-date="<?php echo e($calendarInitialDate->format('Y-m-d')); ?>"
  data-calendar-mode="<?php echo e($calendarMode); ?>"
  data-oncology-track="<?php echo e($oncologyTrack ?? 'infusions'); ?>"
>
  <div class="operational-native-table-heading operational-service-calendar-heading">
    <div>
      <p class="eyebrow">PROGRAMACI&Oacute;N</p>
      <?php if(filled($calendarTitle ?? null)): ?>
        <h2><?php echo e($calendarTitle); ?></h2>
        <span><?php echo e($calendarSubtitle ?? 'Agenda de la sala de infusion seleccionada'); ?></span>
      <?php endif; ?>
    </div>
    <?php if(filled($calendarReturnUrl ?? null)): ?>
      <a class="operational-secondary-button" href="<?php echo e($calendarReturnUrl); ?>">Volver a salas</a>
    <?php elseif(! $isHospitalizationCalendar): ?>
  ...
  <div class="operational-service-calendar-filters" aria-label="Filtros del calendario">
    <label>Servicio
      <select data-service-calendar-filter="service">
        ...
        <?php $__currentLoopData = $calendarServices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $service): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($service); ?>"><?php echo e($service); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      </select>
    </label>
    <label><?php echo e($isHospitalizationCalendar ? 'Area / cama' : 'Sala'); ?>

      <select data-service-calendar-filter="room" <?php if(filled($calendarRoomLock)): echo 'disabled'; endif; ?>>
        <?php if (! (filled($calendarRoomLock))): ?><option value="">Todas</option><?php endif; ?>
        <?php $__currentLoopData = $calendarRooms; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $room): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($room); ?>" <?php if($calendarRoomLock === $room): echo 'selected'; endif; ?>><?php echo e($room); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      </select>
    </label>
    <label>M&eacute;dico o responsable
      <select data-service-calendar-filter="doctor">
        <option value="">Todos</option>
        <?php $__currentLoopData = $calendarDoctors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $doctor): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($doctor); ?>"><?php echo e($doctor); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      </select>
    </label>
    <label>Estatus
  ...
  <div class="operational-service-calendar-toolbar">
    <div class="operational-service-calendar-navigation">
      ...
      <select data-service-calendar-month aria-label="Mes">
        <?php $__currentLoopData = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $month): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <option value="<?php echo e($index); ?>" <?php if($calendarInitialDate->month - 1 === $index): echo 'selected'; endif; ?>><?php echo e($month); ?></option>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      </select>
      <select data-service-calendar-year aria-label="A&ntilde;o">
        <?php $__currentLoopData = $calendarYears; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $year): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($year); ?>" <?php if($calendarInitialDate->year === $year): echo 'selected'; endif; ?>><?php echo e($year); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      </select>
  ...
  <div class="operational-service-calendar-scroll">
    <div class="operational-service-calendar-weekdays" aria-hidden="true">
      <?php $__currentLoopData = ['Dom','Lun','Mar','Mi&eacute;','Jue','Vie','S&aacute;b']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $day): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><span><?php echo $day; ?></span><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
  ...
  <script type="application/json" data-service-calendar-events><?php echo json_encode($calendarEvents, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT); ?></script>
  ...
  <dialog class="operational-service-calendar-dialog operational-oncology-mixture-dialog" data-mixture-detail-dialog aria-labelledby="mixture-request-title">
    <header>
      <div>
        <small><?php echo e($isHospitalizationCalendar ? 'HOSPITALIZACIÓN' : 'CENTRO ONCOLÓGICO'); ?></small>
        <h3 id="mixture-request-title" data-mixture-dialog-title><?php echo e($isHospitalizationCalendar ? 'Solicitud de mezcla hospitalaria' : 'Solicitud de mezcla oncológica'); ?></h3>
      </div>
    ...
    <div class="operational-oncology-mixture-body">
      ...
      <section class="operational-oncology-mixture-section">
        ...
        <div class="operational-oncology-mixture-fields">
          ...
          <?php if($isHospitalizationCalendar): ?>
          ...
          <?php else: ?>
            <form class="operational-oncology-mixture-date-form" method="post" data-mixture-schedule-form>
              ...
              <input type="hidden" name="oncology_track" value="<?php echo e($oncologyTrack ?? 'mixes'); ?>">
              <label>Fecha de programaci&oacute;n
                <span>
      ...
      <section class="operational-oncology-mixture-section">
        ...
        <p class="operational-oncology-mixture-note">
          <span aria-hidden="true">i</span>
          <?php echo e($isHospitalizationCalendar ? 'Los medicamentos mostrados corresponden a la solicitud seleccionada.' : 'Cada medicamento de cada renglón y cada día del calendario corresponde a una mezcla distinta que se agrega a la programación.'); ?>
...
<?php if(! $isHospitalizationCalendar && $selectedCalendarRequest): ?>
  <?php $currentAssignment = data_get($selectedCalendarRequest->payload, 'infusion_assignment', []); ?>
  <section class="operational-native-table-card operational-form-card">
    <div class="operational-native-table-heading">
      <div><p class="eyebrow">AGENDAR SALA DE INFUSI&Oacute;N</p><h2><?php echo e($selectedCalendarRequest->patient?->full_name ?? 'Paciente sin nombre'); ?></h2><span><?php echo e($selectedCalendarRequest->external_id ?? 'QT-'.$selectedCalendarRequest->id); ?> · <?php echo e(data_get($selectedCalendarRequest->payload, 'service', 'Oncologia medica')); ?></span></div>
      <a class="operational-secondary-button" href="<?php echo e(route('operational.dashboard', ['area' => 'oncology', 'section' => 'calendar', 'oncology_track' => $oncologyTrack ?? 'infusions', 'unit' => $contextUnit?->id])); ?>">Cerrar</a>
    </div>
    <form class="operational-detail-form" method="post" action="<?php echo e(route('operational.infusion-assignments.update', $selectedCalendarRequest)); ?>">
      ...
      <label>Sala de infusi&oacute;n *
        <select name="procedure_area_id" required>
          ...
          <?php $__currentLoopData = $infusionRooms->where('status', 'active'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $room): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <option value="<?php echo e($room->id); ?>" <?php if((int) old('procedure_area_id', data_get($currentAssignment, 'procedure_area_id')) === $room->id): echo 'selected'; endif; ?>><?php echo e($room->unit_number); ?> · <?php echo e($room->location); ?> · capacidad <?php echo e($room->simultaneous_capacity); ?></option>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </select>
      </label>
      <label>Fecha de aplicaci&oacute;n *<input type="date" name="application_date" required value="<?php echo e(old('application_date', data_get($currentAssignment, 'application_date', $selectedCalendarRequest->required_at?->format('Y-m-d')))); ?>"></label>
      ...
      <div class="span-2 operational-room-availability">
        ...
        <?php $__empty_1 = true; $__currentLoopData = $infusionRooms->where('status', 'active'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $room): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
          <span><b><?php echo e($room->unit_number); ?></b>: <?php echo e($room->schedules->map(fn ($schedule) => ($dayNames[$schedule->day_of_week] ?? 'Dia').' '.substr((string) $schedule->starts_at, 0, 5).'-'.substr((string) $schedule->ends_at, 0, 5))->implode(', ') ?: 'Sin horario disponible'); ?></span>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
          <span>No hay salas activas. Registra una desde Salas de infusi&oacute;n.</span>
        <?php endif; ?>
      </div>
      <button class="operational-primary-button span-2" type="submit" <?php if($infusionRooms->where('status', 'active')->isEmpty()): echo 'disabled'; endif; ?>>Guardar programaci&oacute;n</button>
    </form>
  </section>
<?php endif; ?>
<?php /**PATH C:\laragon\www\dr-sam\resources\views/operational/sections/service-calendar.blade.php ENDPATH**/ ?>