...
@section('content')
  <div class="unit-native-screen">
    ...
    <section class="unit-native-workspace">
      ...
      @if (in_array($section, $catalogSections, true))
        @include('unit.carousels.'.$section)
      @endif
          ...
          @if ($unitServiceContracts->isNotEmpty())
            <section class="unit-service-carousel-card">
              ...
              <div class="unit-service-carousel" data-unit-service-carousel aria-label="Servicios integrales">
                <button type="button"
                        @class(['unit-service-carousel-button', 'unit-service-catalog-link', 'is-active' => $activeServiceContractId === 'catalog'])
            ...
            <div class="unit-service-panels">
              <section class="unit-catalog-screen" data-unit-service-panel="catalog" @if ($activeServiceContractId !== 'catalog') hidden @endif>
                <header class="unit-service-panel-header unit-catalog-info-card">
                  <div class="unit-service-panel-copy">
                    ...
                    <p>Servicios habilitados para {{ $unit->name }}</p>
                    <div class="unit-service-panel-badges">
                      <span class="unit-native-status">Activo</span>
                      <span>{{ $unitServiceContracts->count() }} servicios disponibles</span>
                    </div>
                  </div>
                </header>
                      ...
                      <tbody>
                        @foreach ($unitServiceContracts as $contract)
                          <tr data-unit-service-catalog-row>
                            <td><strong>{{ $contract->service?->name ?? 'Servicio sin nombre' }}</strong></td>
                            <td>{{ $contract->service?->category ?? 'Sin categoría' }}</td>
                            <td>{{ $contract->service?->specialty ?? 'Servicio general' }}</td>
                            <td><span class="unit-native-status">{{ $statusText($contract->status) }}</span></td>
                            <td><button type="button" class="unit-native-button" data-unit-open-service="{{ $contract->id }}">Abrir</button></td>
                          </tr>
                        @endforeach
              ...
              @foreach ($unitServiceContracts as $contract)
                ...
                <section @class([
                         ...
                         data-unit-service-layout="{{ $serviceKey }}"
                  ...
                  <header class="unit-service-panel-header" data-unit-service-info>
                    <div class="unit-service-panel-copy">
                      <h2>{{ $serviceName }}</h2>
                      <p>{{ $isConsultationService ? 'Atencion medica · Consulta Externa' : (($service?->category ?? 'Sin categoria').' · '.($service?->specialty ?? 'Servicio general')) }}</p>
                      <div class="unit-service-panel-badges">
                  ...
                  <div data-unit-service-overview @if ($isConsultationService) data-unit-consultation-view="home" @endif>
                    @if ($isConsultationService)
                      <section class="unit-consultation-calendar"
                        ...
                        <dialog class="unit-consultation-flow-dialog unit-consultation-new-dialog"
                                ...
                                @if ($errors->any() && old('patient_id')) data-open-on-load @endif>
                          <form method="post" action="{{ route('unit.appointments.store') }}" class="unit-consultation-flow-shell" data-unit-calendar-new-form>
                            @csrf
                            <input type="hidden" name="unit" value="{{ $unit->id }}">
                            <input type="hidden" name="patient_id" value="{{ old('patient_id') }}" data-unit-new-field="patient_id">
                            ...
                            <p class="unit-consultation-flow-error" data-unit-new-error @if (! ($errors->any() && old('patient_id'))) hidden @endif>{{ $errors->first() }}</p>
                            ...
                            <div class="unit-consultation-booking-layout" data-unit-new-reference-layout>
                              <div class="unit-consultation-booking-column is-main">
                                ...
                                <section class="unit-consultation-booking-section is-consultation-data">
                                  ...
                                  <div class="unit-consultation-modality-options">
                                    <label>
                                      <input type="radio" name="modality" value="Presencial" @checked(old('modality', 'Presencial') === 'Presencial')>
                                    ...
                                    <label>
                                      <input type="radio" name="modality" value="Video llamada" @checked(old('modality') === 'Video llamada')>
                                  ...
                                  <div class="unit-consultation-details-grid">
                                    ...
                                    <label class="is-wide"><span>Motivo de la consulta <b>*</b></span><textarea name="reason" data-unit-new-field="reason" maxlength="500" rows="2" required placeholder="Escribe brevemente el motivo de la cita">{{ old('reason') }}</textarea><small><span data-unit-new-counter="reason">{{ mb_strlen((string) old('reason')) }}</span>/500</small></label>
                                    <label class="is-wide"><span>Notas adicionales</span><textarea name="notes" maxlength="500" rows="2" placeholder="Indicaciones o informaci&oacute;n adicional...">{{ old('notes') }}</textarea><small><span data-unit-new-counter="notes">{{ mb_strlen((string) old('notes')) }}</span>/500</small></label>
                                  </div>
                              ...
                              <aside class="unit-consultation-booking-column is-schedule" aria-label="Fecha, horario y confirmacion">
                                <section class="unit-consultation-booking-section is-date-time">
                                  ...
                                  <label class="unit-consultation-date-field"><span>Fecha de la cita <b>*</b></span><input type="date" name="appointment_date" data-unit-new-field="appointment_date" min="{{ now()->toDateString() }}" value="{{ old('appointment_date', now()->toDateString()) }}" required></label>
                                ...
                                <section class="unit-consultation-booking-section is-confirmation">
                                  ...
                                  <fieldset class="unit-consultation-notifications">
                                    ...
                                    <label><input type="hidden" name="notify_email" value="0"><input type="checkbox" name="notify_email" value="1" @checked((bool) old('notify_email', true))><span>Enviar confirmaci&oacute;n por correo<small data-unit-new-notification-note>Incluye los datos de la consulta.</small></span></label>
                                    <label><input type="hidden" name="notify_sms" value="0"><input type="checkbox" name="notify_sms" value="1" @checked((bool) old('notify_sms', true))><span>Enviar confirmaci&oacute;n por SMS<small>Se utilizar&aacute; el tel&eacute;fono registrado.</small></span></label>
                                  </fieldset>
                          ...
                          <table class="unit-native-table unit-nutrition-request-table">
                            ...
                            <tbody>
                              @forelse ($serviceProviderRequests as $nutritionRequest)
                                ...
                                @endphp
                                <tr data-unit-nutrition-row data-unit-request-row data-category="{{ $nutritionCategory }}">
                    ...
                    @else
                      <section class="unit-service-generic-board" data-unit-request-board>
                        <div class="unit-nutrition-request-toolbar" data-unit-service-controls>
                          <div class="unit-nutrition-request-tabs" role="tablist" aria-label="Filtrar solicitudes de {{ $serviceName }}">
                            ...
                            <button type="button" data-unit-request-filter="preparing,route" role="tab" aria-selected="false">
                              En proceso <span>{{ $nutritionRequestCounts->get('preparing', 0) + $nutritionRequestCounts->get('route', 0) }}</span>
                            </button>
                            <button type="button" data-unit-request-filter="delivered" role="tab" aria-selected="false">
                              Completadas <span>{{ $nutritionRequestCounts->get('delivered', 0) }}</span>
                            </button>
                            <button type="button" data-unit-request-filter="history" role="tab" aria-selected="false">Historial</button>
                          </div>
                          <a class="unit-nutrition-new-request" href="{{ $operationHref }}">
                        ...
                        <div class="unit-nutrition-request-table-wrap unit-service-generic-table-wrap" data-unit-service-content data-drsam-table-filter-skip>
                          ...
                          <tbody>
                            @forelse ($serviceProviderRequests as $serviceProviderRequest)
                              ...
                              @endphp
                              <tr data-unit-request-row data-category="{{ $genericCategory }}">
                                <td><strong>{{ $genericFolio }}</strong></td>
                                <td>{{ $genericType }}</td>
                                <td><strong>{{ $genericHospital }}</strong><small>{{ $unit->clues ?? $unit->code ?? 'Sin CLUES' }}</small></td>
                                ...
                                <td><span class="unit-consultation-chip is-status-{{ $genericStatusClass }}">{{ $genericStatusLabel }}</span></td>
                                <td><a class="unit-nutrition-request-view" href="{{ $genericRoute }}" aria-label="Ver solicitud {{ $genericFolio }}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6-10-6-10-6Z"/><circle cx="12" cy="12" r="3"/></svg>Ver</a></td>
                              </tr>
                            @empty
                              <tr data-unit-request-empty><td colspan="9">No se encontraron solicitudes para {{ $serviceName }}.</td></tr>
                            @endforelse
                            @if ($serviceProviderRequests->isNotEmpty())
        ...
        <div class="unit-catalog-screen" data-unit-catalog-screen>
          <section class="unit-service-panel-header unit-catalog-info-card" data-unit-catalog-info>
            <div class="unit-service-panel-copy">
              ...
              <div class="unit-service-panel-badges">
                <span class="unit-native-status">Activo</span>
                <span>{{ $catalogModules->count() }} catalogos disponibles</span>
              </div>
            </div>
          </section>
              ...
              <table class="unit-native-table unit-catalog-overview-table" data-unit-catalog-table>
                ...
                <tbody>
                  @foreach ($catalogModules as $module)
                    <tr data-unit-catalog-row data-group="{{ $module['group'] }}" data-status="active">
                      <td><strong>{{ $module['name'] }}</strong><small>{{ $module['description'] }}</small></td>
                      <td>{{ $module['group_label'] }}</td>
                      <td>{{ $module['count'] }}</td>
                      <td><span class="unit-native-status">Disponible</span></td>
                      <td><a class="unit-native-button" href="{{ route('unit.dashboard', ['unit' => $unit->id, 'section' => $module['section']]) }}">Abrir</a></td>
                    </tr>
                  @endforeach
        ...
        <div class="unit-catalog-screen" data-unit-catalog-screen>
          <section class="unit-service-panel-header unit-catalog-info-card" data-unit-catalog-info>
            <div class="unit-service-panel-copy">
              ...
              <div class="unit-service-panel-badges">
                <span class="unit-native-status">Activo</span>
                <span>{{ $unit->operationalProfiles->count() }} usuarios registrados</span>
              </div>
            </div>
          </section>
          ...
          <div class="unit-catalog-toolbar" data-unit-catalog-controls>
            <div class="unit-nutrition-request-tabs" role="tablist" aria-label="Filtrar usuarios operativos">
              ...
              <button type="button" data-unit-catalog-status="active" role="tab" aria-selected="false">Activos <span>{{ $unit->operationalProfiles->where('status', 'active')->count() }}</span></button>
              <button type="button" data-unit-catalog-status="inactive" role="tab" aria-selected="false">Inactivos <span>{{ $unit->operationalProfiles->where('status', 'inactive')->count() }}</span></button>
            </div>
            <div class="unit-catalog-toolbar-actions">
              <label class="unit-catalog-search">
          ...
          <section class="unit-native-table-card">
            <div class="unit-native-table-heading">
              <div><h2>Usuarios de la unidad</h2><p>{{ $unit->operationalProfiles->count() }} usuarios</p></div>
            </div>
            <div class="unit-native-user-list">
              @forelse ($unit->operationalProfiles as $profile)
                <article data-operational-user-row data-unit-catalog-row data-status="{{ $profile->status }}">
                  <div>
        ...
        <div class="unit-catalog-screen" data-unit-catalog-screen>
          <section class="unit-service-panel-header unit-catalog-info-card" data-unit-catalog-info>
            <div class="unit-service-panel-copy">
              ...
              <div class="unit-service-panel-badges">
                <span class="unit-native-status">Activo</span>
                <span>{{ $patients->count() }} pacientes registrados</span>
              </div>
            </div>
          </section>
          ...
          <div class="unit-catalog-toolbar" data-unit-catalog-controls>
            <div class="unit-nutrition-request-tabs" role="tablist" aria-label="Filtrar pacientes">
              <button type="button" class="is-active" data-unit-catalog-status="all" role="tab" aria-selected="true">Todos</button>
              <button type="button" data-unit-catalog-status="active" role="tab" aria-selected="false">Activos <span>{{ $patients->where('status', 'active')->count() }}</span></button>
              <button type="button" data-unit-catalog-status="inactive" role="tab" aria-selected="false">Inactivos <span>{{ $patients->where('status', 'inactive')->count() }}</span></button>
            </div>
            <div class="unit-catalog-toolbar-actions">
              <label class="unit-catalog-search">
              ...
              <table class="unit-native-table" data-unit-catalog-table>
                ...
                <tbody>
                  @forelse ($patients as $patient)
                    ...
                    <tr data-unit-catalog-row data-status="{{ $patient->status }}">
      ...
      @if ($section === 'doctors')
        <div class="unit-catalog-screen" data-unit-catalog-screen>
          <section class="unit-service-panel-header unit-catalog-info-card" data-unit-catalog-info>
            <div class="unit-service-panel-copy">
              ...
              <div class="unit-service-panel-badges">
                <span class="unit-native-status">Activo</span>
                <span>{{ $unit->doctors->count() }} medicos adscritos</span>
              </div>
            </div>
          </section>
          ...
          <div class="unit-catalog-toolbar" data-unit-catalog-controls>
            <div class="unit-nutrition-request-tabs" role="tablist" aria-label="Filtrar medicos">
              <button type="button" class="is-active" data-unit-catalog-status="all" role="tab" aria-selected="true">Todos</button>
              <button type="button" data-unit-catalog-status="active" role="tab" aria-selected="false">Activos <span>{{ $unit->doctors->where('status', 'active')->count() }}</span></button>
              <button type="button" data-unit-catalog-status="pending" role="tab" aria-selected="false">Pendientes <span>{{ $unit->doctors->where('status', 'pending')->count() }}</span></button>
              <button type="button" data-unit-catalog-status="inactive" role="tab" aria-selected="false">Inactivos <span>{{ $unit->doctors->where('status', 'inactive')->count() }}</span></button>
            </div>
            <div class="unit-catalog-toolbar-actions">
              <label class="unit-catalog-search">
        ...
        <section class="unit-native-table-card unit-catalog-content-card" data-unit-catalog-content>
          <div class="unit-native-table-scroll">
            <table class="unit-native-table unit-doctors-table" data-unit-catalog-table>
              ...
              <tbody>
                @forelse ($unit->doctors as $doctor)
                  <tr data-doctor-row data-unit-catalog-row data-status="{{ $doctor->status }}">
                    ...
                    <td><span @class(['unit-native-status', 'is-warning' => $doctor->status === 'pending'])>{{ $statusText($doctor->status) }}</span></td>
        ...
        <dialog class="unit-doctor-create-dialog" data-doctor-dialog aria-labelledby="unit-doctor-dialog-title">
          <form method="post" action="{{ route('unit.doctors.store') }}" class="unit-doctor-dialog-form">
            @csrf
            ...
            <div class="unit-doctor-dialog-body">
              @if (old('_doctor_form') && $errors->any())
                <div class="unit-doctor-dialog-errors" role="alert"><strong>No se pudo guardar el medico.</strong><ul>@foreach ($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div>
              @endif
              <label><span>Nombre <em aria-hidden="true">*</em></span><input name="first_name" value="{{ old('first_name') }}" placeholder="Ej. Juan" required></label>
              <label><span>Apellido <em aria-hidden="true">*</em></span><input name="last_name" value="{{ old('last_name') }}" placeholder="Ej. Perez" required></label>
              <label><span>Usuario de la plataforma</span><input name="platform_user" value="{{ old('platform_user') }}" placeholder="Ej. juan.perez"></label>
              <label><span>Cedula profesional <em aria-hidden="true">*</em></span><input name="professional_license" value="{{ old('professional_license') }}" placeholder="Ej. 12345678" required></label>
              <label><span>Especialidad <em aria-hidden="true">*</em></span><select name="specialty" required><option value="">Selecciona una especialidad</option>@foreach ($specialties->pluck('specialty')->filter()->unique()->sort() as $specialty)<option value="{{ $specialty }}" @selected(old('specialty') === $specialty)>{{ $specialty }}</option>@endforeach</select></label>
              <label>Subespecialidad<input name="subspecialty" value="{{ old('subspecialty') }}"></label>
              <fieldset class="wide">
                <legend>Servicio adscrito <em aria-hidden="true">*</em></legend>
                <div class="unit-native-choice-grid">
                  @forelse ($unit->contractedServices as $contract)
                    <label><input type="checkbox" name="services[]" value="{{ $contract->service?->name }}" @checked(in_array($contract->service?->name, (array) old('services', old('_doctor_form') ? [] : [$unit->contractedServices->first()?->service?->name]), true))><span><strong>{{ $contract->service?->name }}</strong><small>Servicio habilitado para adscripcion medica.</small></span></label>
                  @empty
                    @foreach ($serviceChoices as $choice)<label><input type="checkbox" name="services[]" value="{{ $choice }}" @checked(in_array($choice, (array) old('services', old('_doctor_form') ? [] : [$serviceChoices[0] ?? null]), true))><span><strong>{{ $choice }}</strong><small>Servicio habilitado para adscripcion medica.</small></span></label>@endforeach
                  @endforelse
                </div>
              </fieldset>
            </div>
        ...
        <div class="unit-catalog-screen" data-unit-catalog-screen>
          <section class="unit-service-panel-header unit-catalog-info-card" data-unit-catalog-info>
            <div class="unit-service-panel-copy">
              ...
              <div class="unit-service-panel-badges">
                <span class="unit-native-status">Activo</span>
                <span>{{ $specialties->count() }} especialidades habilitadas</span>
              </div>
            </div>
          </section>
          ...
          <div class="unit-catalog-toolbar" data-unit-catalog-controls>
            <div class="unit-nutrition-request-tabs" role="tablist" aria-label="Filtrar especialidades">
              ...
              <button type="button" data-unit-catalog-status="active" role="tab" aria-selected="false">Activas <span>{{ $specialties->where('status', 'active')->count() }}</span></button>
              <button type="button" data-unit-catalog-status="inactive" role="tab" aria-selected="false">Inactivas <span>{{ $specialties->where('status', 'inactive')->count() }}</span></button>
            </div>
            <div class="unit-catalog-toolbar-actions">
              <label class="unit-catalog-search">
              ...
              <table class="unit-native-table" data-unit-catalog-table>
                ...
                <tbody>
                  @forelse ($specialties as $service)
                    <tr data-unit-catalog-row data-status="{{ $service->status }}">
      ...
      @if ($section === 'procedure-areas')
        ...
        <div class="unit-catalog-screen unit-procedure-screen" data-unit-catalog-screen>
          <section class="unit-service-panel-header unit-catalog-info-card" data-unit-catalog-info>
            <div class="unit-service-panel-copy">
              ...
              <div class="unit-service-panel-badges">
                <span class="unit-native-status">Activo</span>
                <span>{{ $allProcedureAreas->count() }} subunidades registradas</span>
              </div>
            </div>
          </section>
          ...
          <div class="unit-catalog-toolbar unit-procedure-toolbar" data-unit-catalog-controls>
            <div class="unit-nutrition-request-tabs" data-procedure-carousel role="tablist" aria-label="Filtrar areas de procedimiento">
              <button type="button"
                      @class(['is-active' => $activeProcedureCatalog === 'all'])
                      data-procedure-category="all"
                      role="tab"
                      aria-selected="{{ $activeProcedureCatalog === 'all' ? 'true' : 'false' }}">Todos</button>
              @foreach ($procedureCatalogs as $catalogKey => $catalog)
                <button type="button"
                        @class(['is-active' => $catalogKey === $activeProcedureCatalog])
                        data-procedure-category="{{ $catalogKey }}"
                        role="tab"
                        aria-selected="{{ $catalogKey === $activeProcedureCatalog ? 'true' : 'false' }}">{{ $catalog['button'] }}</button>
              @endforeach
            </div>
            <div class="unit-catalog-toolbar-actions">
              <label class="unit-catalog-search">
              ...
              @foreach ($procedureCatalogs as $catalogKey => $catalog)
                <a class="unit-nutrition-new-request"
                   href="{{ route('unit.dashboard', ['unit' => $unit->id, 'section' => 'procedure-areas', 'create' => $catalogKey]) }}"
                   data-procedure-create
                   data-procedure-create-toolbar="{{ $catalogKey }}"
                   data-procedure-create-category="{{ $catalogKey }}"
                   data-procedure-create-label="{{ ucfirst($catalog['singular']) }}"
                   @if (($activeProcedureCatalog === 'all' ? array_key_first($procedureCatalogs) : $activeProcedureCatalog) !== $catalogKey) hidden @endif><span aria-hidden="true">+</span>{{ $catalog['create'] }}</a>
              @endforeach
            </div>
          </div>

        <div class="unit-procedure-catalogs" data-unit-catalog-content>
          <section class="unit-native-table-card unit-procedure-card" data-procedure-catalog="all" @if ($activeProcedureCatalog !== 'all') hidden @endif>
            <div class="unit-native-table-scroll">
              <table class="unit-native-table unit-procedure-table">
                ...
                <tbody>
                  @forelse ($allProcedureAreas as $area)
                    ...
                    <tr data-procedure-row><td>{{ $area['location'] }}</td><td>{{ $area['floor'] }}</td><td><strong>{{ $area['unit_number'] }}</strong><small>{{ $areaCatalog['button'] ?? 'Subunidad' }}</small></td><td>{{ $area['capacity'] }} {{ $area['capacity'] == 1 ? 'paciente simultaneo' : 'pacientes simultaneos' }}</td><td>{{ $area['responsible'] }}</td><td>{{ $activeSchedule ?: 'Sin horario' }}</td><td><a class="unit-native-button" href="{{ route('unit.dashboard', ['unit' => $unit->id, 'section' => 'procedure-areas', 'edit' => $area['id']]) }}" data-procedure-edit data-procedure-edit-id="{{ $area['id'] }}" data-procedure-edit-action="{{ route('unit.procedure-areas.update', $area['id']) }}" data-procedure-edit-label="{{ $areaCatalog['singular'] ?? 'subunidad' }}">Editar</a></td></tr>
                  @empty
          ...
          @foreach ($procedureCatalogs as $catalogKey => $catalog)
            ...
            <section class="unit-native-table-card unit-procedure-card" data-procedure-catalog="{{ $catalogKey }}" @if ($catalogKey !== $activeProcedureCatalog) hidden @endif>
              <div class="unit-native-table-scroll">
                <table class="unit-native-table unit-procedure-table">
                  <caption class="sr-only">{{ $catalog['title'] }}</caption>
                  <thead><tr><th>Ubicacion</th><th>Piso</th><th>Numero de unidad</th><th>Capacidad simultanea</th><th>Responsable de area</th><th>Horario de atencion</th><th>Acciones</th></tr></thead>
                  <tbody>
                    @forelse ($catalogAreas as $area)
                      @php($activeSchedule = collect($area['schedule'] ?? [])->filter(fn ($day) => data_get($day, 'enabled'))->map(fn ($day, $key) => ($scheduleDays[$key] ?? ucfirst($key)).' '.data_get($day, 'start', '08:00').' - '.data_get($day, 'end', '16:00'))->implode(', '))
                      <tr data-procedure-row><td>{{ $area['location'] }}</td><td>{{ $area['floor'] }}</td><td><strong>{{ $area['unit_number'] }}</strong><small>{{ $catalog['button'] }}</small></td><td>{{ $area['capacity'] }} {{ $area['capacity'] == 1 ? 'paciente simultaneo' : 'pacientes simultaneos' }}</td><td>{{ $area['responsible'] }}</td><td>{{ $activeSchedule ?: 'Sin horario' }}</td><td><a class="unit-native-button" href="{{ route('unit.dashboard', ['unit' => $unit->id, 'section' => 'procedure-areas', 'edit' => $area['id']]) }}" data-procedure-edit data-procedure-edit-id="{{ $area['id'] }}" data-procedure-edit-action="{{ route('unit.procedure-areas.update', $area['id']) }}" data-procedure-edit-label="{{ $catalog['singular'] }}">Editar</a></td></tr>
                    @empty
        ...
        <dialog class="unit-procedure-dialog"
                ...
                @if ($openProcedureCreateDialog) data-open-on-load @endif>
          <form method="post" action="{{ route('unit.procedure-areas.store') }}" class="unit-procedure-dialog-form" data-procedure-create-form>
            @csrf
            <header class="unit-procedure-dialog-header">
              ...
              <div>
                ...
                <span data-procedure-create-dialog-label>{{ ucfirst($procedureCatalogs[$initialProcedureCreateCatalog]['singular']) }}</span>
              </div>
            ...
            <div class="unit-procedure-dialog-body">
              <input type="hidden" name="unit" value="{{ $unit->id }}">
              <input type="hidden" name="type" value="{{ $initialProcedureCreateCatalog }}" data-procedure-create-type>

              @if ($errors->any() && array_key_exists($oldProcedureCatalog, $procedureCatalogs))
              ...
              <label>Ubicacion<input name="location" value="{{ old('location') }}" maxlength="180" autocomplete="off" autofocus required></label>
              <label>Piso<input name="floor" value="{{ old('floor') }}" maxlength="40" autocomplete="off" required></label>
              <label>Numero de unidad<input name="unit_number" value="{{ old('unit_number') }}" maxlength="80" autocomplete="off" required></label>
              <label>Capacidad simultanea de pacientes<input type="number" min="1" max="999" name="capacity" value="{{ old('capacity') }}" required></label>
              <label class="wide">Responsable de area<select name="responsible"><option value="" @selected(old('responsible', '') === '')>Sin responsable asignado</option>@foreach ($unit->operationalProfiles as $profile)<option value="{{ $profile->user?->name }}" @selected(old('responsible') === $profile->user?->name)>{{ $profile->user?->name }}</option>@endforeach</select></label>

              <fieldset class="wide">
                <legend>Horario de atencion de la unidad</legend>
                <div class="unit-procedure-schedule">
                  @foreach ($scheduleDays as $dayKey => $dayLabel)
                    @php($dayEnabled = (bool) old("schedule.$dayKey.enabled", $loop->iteration <= 5))
                    <div @class(['is-enabled' => $dayEnabled]) data-procedure-schedule-day>
                      <label><input type="hidden" name="schedule[{{ $dayKey }}][enabled]" value="0"><input type="checkbox" name="schedule[{{ $dayKey }}][enabled]" value="1" @checked($dayEnabled) data-procedure-schedule-toggle>{{ $dayLabel }}</label>
                      <span aria-hidden="true">+</span>
                      <label>Hora inicio<input type="time" name="schedule[{{ $dayKey }}][start]" value="{{ old("schedule.$dayKey.start", '08:00') }}"></label>
                      <label>Hora fin<input type="time" name="schedule[{{ $dayKey }}][end]" value="{{ old("schedule.$dayKey.end", '16:00') }}"></label>
                    </div>
                  @endforeach
                </div>
              </fieldset>
        ...
        <dialog class="unit-procedure-dialog"
                ...
                @if ($openProcedureEditDialog) data-open-on-load @endif>
          <form method="post"
                action="{{ $editingProcedureArea ? route('unit.procedure-areas.update', data_get($editingProcedureArea, 'id')) : '' }}"
            ...
            <header class="unit-procedure-dialog-header">
              ...
              <div>
                <strong id="unit-procedure-edit-dialog-title" data-procedure-edit-title>Editar {{ $editingProcedureLabel }}</strong>
            ...
            <div class="unit-procedure-dialog-body">
              <input type="hidden" name="unit" value="{{ $unit->id }}">
              <input type="hidden" name="type" value="{{ old('type', $editingProcedureCatalog) }}" data-procedure-edit-type>

              @if ($openProcedureEditDialog && $errors->any())
              ...
              <label>Ubicacion<input name="location" value="{{ old('location', data_get($editingProcedureArea, 'location', '')) }}" maxlength="180" autocomplete="off" autofocus required></label>
              <label>Piso<input name="floor" value="{{ old('floor', data_get($editingProcedureArea, 'floor', '')) }}" maxlength="40" autocomplete="off" required></label>
              <label>Numero de unidad<input name="unit_number" value="{{ old('unit_number', data_get($editingProcedureArea, 'unit_number', '')) }}" maxlength="80" autocomplete="off" required></label>
              <label>Capacidad simultanea de pacientes<input type="number" min="1" max="999" name="capacity" value="{{ old('capacity', data_get($editingProcedureArea, 'capacity', '')) }}" required></label>
              <label class="wide">Responsable de area<select name="responsible"><option value="" @selected(in_array($editingResponsible, ['', 'Sin responsable'], true))>Sin responsable asignado</option>@foreach ($unit->operationalProfiles as $profile)<option value="{{ $profile->user?->name }}" @selected($editingResponsible === $profile->user?->name)>{{ $profile->user?->name }}</option>@endforeach</select></label>
              ...
              <fieldset class="wide">
                ...
                <div class="unit-procedure-schedule">
                  @foreach ($scheduleDays as $dayKey => $dayLabel)
                    ...
                    <div @class(['is-enabled' => $dayEnabled]) data-procedure-edit-schedule-day data-procedure-day="{{ $dayKey }}">
                      <label><input type="hidden" name="schedule[{{ $dayKey }}][enabled]" value="0"><input type="checkbox" name="schedule[{{ $dayKey }}][enabled]" value="1" @checked($dayEnabled) data-procedure-edit-schedule-toggle>{{ $dayLabel }}</label>
                      <span aria-hidden="true">+</span>
                      <label>Hora inicio<input type="time" name="schedule[{{ $dayKey }}][start]" value="{{ old("schedule.$dayKey.start", data_get($editingProcedureArea, "schedule.$dayKey.start", '08:00')) }}"></label>
                      <label>Hora fin<input type="time" name="schedule[{{ $dayKey }}][end]" value="{{ old("schedule.$dayKey.end", data_get($editingProcedureArea, "schedule.$dayKey.end", '16:00')) }}"></label>
                    </div>
                  @endforeach
                </div>
              </fieldset>
        ...
        <script>
          (() => {
            ...
            const procedureAreaData = {{ Illuminate\Support\Js::from($procedureAreaPayload) }};
            ...
            const setActiveCatalog = (key) => {
              ...
              const actionKey = key === 'all' ? @json(array_key_first($procedureCatalogs)) : key;
        ...
        <div class="unit-catalog-screen" data-unit-catalog-screen>
          <section class="unit-service-panel-header unit-catalog-info-card" data-unit-catalog-info>
            <div class="unit-service-panel-copy">
              ...
              <div class="unit-service-panel-badges">
                <span class="unit-native-status">Activo</span>
                <span><strong data-pharmacy-visible data-unit-catalog-visible>{{ $externalPharmacyCatalog->count() }}</strong> de {{ $externalPharmacyCatalog->count() }} medicamentos visibles</span>
              </div>
            </div>
          </section>
          ...
          <div class="unit-catalog-toolbar" data-unit-catalog-controls>
            <div class="unit-nutrition-request-tabs" role="tablist" aria-label="Filtrar farmacia externa">
              <button type="button" class="is-active" data-unit-catalog-status="all" role="tab" aria-selected="true">Todos</button>
              <button type="button" data-unit-catalog-status="active" role="tab" aria-selected="false">Activos <span>{{ $externalPharmacyCatalog->where('status', 'active')->count() }}</span></button>
              <button type="button" data-unit-catalog-status="inactive" role="tab" aria-selected="false">Inactivos <span>{{ $externalPharmacyCatalog->where('status', 'inactive')->count() }}</span></button>
            </div>
            <div class="unit-catalog-toolbar-actions">
              <label class="unit-catalog-search">
              ...
              <table class="unit-native-table unit-external-pharmacy-table" data-pharmacy-table data-unit-catalog-table>
                ...
                <tbody>
                  @forelse ($externalPharmacyCatalog as $item)
                    <tr data-pharmacy-row data-unit-catalog-row data-status="{{ $item->status }}">
                      <td>{{ $item->cnis ?? 'Sin CNIS' }}</td>
                      <td><strong>{{ $item->name }}</strong><small>Farmacia Externa</small></td>
      ...
      @if ($section === 'medications')
        @include('unit._medications_catalog')
      @endif

      @if (in_array($section, $catalogSections, true) && $section !== 'medications')
        <script>
          (() => {
            document.querySelectorAll('[data-unit-catalog-screen]').forEach((root) => {
              ...
              root.querySelector('[data-unit-catalog-export]')?.addEventListener('click', () => {
                ...
                const link = document.createElement('a');
                link.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' }));
                link.download = `catalogo-${@json($section)}.csv`;
                link.click();
                URL.revokeObjectURL(link.href);
              });
