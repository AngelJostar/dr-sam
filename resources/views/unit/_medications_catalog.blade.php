...
<div class="unit-catalog-screen unit-medication-screen" data-unit-catalog-screen data-medication-catalog>
  <section class="unit-service-panel-header unit-catalog-info-card" data-unit-catalog-info>
    <div class="unit-service-panel-copy">
      <h2>Medicamentos</h2>
      <p>Catálogo institucional de {{ $unit->institution?->name ?? 'la unidad' }}</p>
      @if ($medicationCatalog->contains(fn ($item) => data_get($item->metadata, 'source_label') === 'Listado de Insumos Esenciales, IMSS Bienestar 2026'))
      ...
      <div class="unit-service-panel-badges">
        ...
        <span><strong data-medication-visible>{{ $medicationCatalog->count() }}</strong> de {{ $medicationCatalog->count() }} medicamentos visibles</span>
      </div>
    </div>
  </section>
  ...
  <div class="unit-catalog-toolbar" data-unit-catalog-controls>
    <div class="unit-nutrition-request-tabs" role="tablist" aria-label="Filtrar medicamentos">
      ...
      <button type="button" data-unit-catalog-status="active" role="tab" aria-selected="false">Activos <span>{{ $activeMedicationCount }}</span></button>
      <button type="button" data-unit-catalog-status="inactive" role="tab" aria-selected="false">Inactivos <span>{{ $medicationCatalog->count() - $activeMedicationCount }}</span></button>
    </div>
  ...
  <section class="unit-native-table-card unit-catalog-content-card unit-medication-card" data-unit-catalog-content>
    <div class="unit-native-table-scroll">
      <table class="unit-native-table unit-medication-catalog-table" data-medication-table>
        <thead>
          <tr>
            @foreach ($medicationColumns as $columnKey => $columnLabel)
              <th scope="col">
                <span class="unit-medication-column-heading">
                  <span>{{ $columnLabel }}</span>
                  <button type="button" class="unit-medication-filter-trigger"
                          data-medication-filter-trigger="{{ $columnKey }}"
                          aria-label="Buscar y filtrar {{ $columnLabel }}"
                          aria-expanded="false"
                          title="Buscar y filtrar {{ $columnLabel }}">
        ...
        <tbody>
          @forelse ($medicationCatalog as $item)
            ...
            @endphp
            <tr data-medication-row data-status="{{ $isActive ? 'active' : 'inactive' }}">
              <td data-medication-cell="status" data-medication-value="{{ $isActive ? 'Activo' : 'Inactivo' }}">
                @if ($item->status === 'active')
                  <form method="post" action="{{ route('unit.medications.status', ['medication' => $item, 'unit' => $unit->id]) }}" data-medication-toggle>
                    @csrf @method('patch')
                    <input type="hidden" name="is_active" value="{{ $isActive ? '0' : '1' }}">
                    <button type="submit" @class(['unit-medication-switch', 'is-on' => $isActive])
                            role="switch" aria-checked="{{ $isActive ? 'true' : 'false' }}"
                            aria-label="{{ $isActive ? 'Desactivar' : 'Activar' }} {{ $item->name }} en esta unidad"
                            title="{{ $isActive ? 'Desactivar' : 'Activar' }} en esta unidad">
              ...
              <td data-medication-cell="product" data-medication-value="{{ $item->name }}" data-medication-search-value="{{ $item->name }} {{ $item->cnis }}">
                <strong>{{ $item->name }}</strong><small>{{ $item->cnis ?? 'Sin clave' }}</small>
              </td>
              <td data-medication-cell="presentation" data-medication-value="{{ $item->presentation ?? 'No especificada' }}">
                {{ $item->presentation ?? 'No especificada' }}
                @if ($package)<small>{{ $package }}</small>@endif
              </td>
              <td data-medication-cell="group" data-medication-value="{{ $item->therapeutic_group ?? 'Sin grupo' }}">{{ $item->therapeutic_group ?? 'Sin grupo' }}</td>
              <td data-medication-cell="condition" data-medication-value="{{ $condition }}">{{ $condition }}</td>
              <td data-medication-cell="stock" data-medication-value="{{ $stock === null ? 'Sin registro' : $stock }}">
                {{ $stock === null ? 'Sin registro' : number_format((int) $stock) }}
              </td>
              <td data-medication-cell="actions" data-medication-value="{{ $item->status === 'active' ? 'Disponible' : 'Bloqueado' }}">
                <button type="button" class="unit-medication-view" data-medication-view
                        data-name="{{ $item->name }}"
                        data-cnis="{{ $item->cnis }}"
                        data-group="{{ $item->therapeutic_group }}"
                        data-description="{{ $item->description }}"
                        data-source="{{ data_get($item->metadata, 'source_label', 'Registro institucional') }}"
                        aria-label="Ver {{ $item->name }}" title="Ver medicamento">
