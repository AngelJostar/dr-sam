...
@section('body_class', 'external-pharmacy-native-body')
    ...
    <aside class="external-pharmacy-native-sidebar" aria-label="Navegacion farmacia externa">
      ...
      <nav class="external-pharmacy-native-menu">
        @foreach ($menu as $key => [$icon, $label])
          <a @class(['is-active' => $section === $key]) href="{{ route('external-pharmacy.dashboard', ['section' => $key]) }}"><span>{{ $icon }}</span>{{ $label }}</a>
        @endforeach
      </nav>

      <a class="external-pharmacy-native-back" href="{{ route('dashboard') }}">â† Modulo de acceso</a>
    </aside>
      ...
      <header class="external-pharmacy-native-header">
        ...
        <div class="external-pharmacy-native-selectors">
          <label>
            Operar como
            <select>
              <option>{{ $areaName }} - {{ $unitName }}</option>
            </select>
          </label>
          <label>
            Unidad
      ...
      @if (in_array($section, ['pending', 'filled'], true))
        <section class="external-pharmacy-native-card">
          <div class="external-pharmacy-native-heading">
            <div>
              <h2>{{ $section === 'filled' ? 'Recetas surtidas' : 'Recetas activas' }}</h2>
                @php
        ...
        <section class="external-pharmacy-native-card">
          <div class="external-pharmacy-native-heading external-heading-action">
            <div>
              ...
              <p>{{ $inventory->count() }} visibles - {{ $products->count() }} medicamentos</p>
            </div>
