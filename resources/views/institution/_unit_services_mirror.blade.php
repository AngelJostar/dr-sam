...
@foreach ($institution->medicalUnits as $mirrorUnit)
  @php
  ...
  @endphp
  <dialog class="institution-unit-services-dialog" data-unit-services-dialog="{{ $mirrorUnit->id }}">
    <header>
      <div>
        <strong>Servicios activos</strong>
        <h2>{{ $mirrorUnit->name }}</h2>
        <p>{{ $mirrorUnit->clues ?? $mirrorUnit->code ?? 'Sin CLUES' }} - {{ $mirrorUnitLocation ?: 'Sin ubicacion' }}</p>
      </div>
    ...
    <div class="institution-unit-services-dialog-body">
      @if ($mirrorContracts->isNotEmpty())
        <div class="unit-service-dashboard institution-unit-service-mirror" data-institution-unit-service-dashboard="{{ $mirrorUnit->id }}">
          <section class="unit-service-carousel-card">
            ...
            <div class="unit-service-carousel" data-unit-service-carousel aria-label="Servicios habilitados">
              @foreach ($mirrorContracts as $mirrorContract)
                @php
                ...
                @endphp
                <button type="button"
                        @class(['unit-service-carousel-button', 'is-active' => $loop->first])
                        data-unit-service-tab="{{ $mirrorServiceKey }}">
                  <span class="unit-service-carousel-icon is-{{ $mirrorServiceIcon }}" aria-hidden="true">
                    @switch($mirrorServiceIcon)
                      @case('consultation')
                  ...
                  <strong>{{ $mirrorServiceName }}</strong>
                </button>
              @endforeach
            </div>
          ...
          <div class="unit-service-panels">
            @foreach ($mirrorContracts as $mirrorContract)
              ...
              @endphp
              <section class="unit-service-panel" data-unit-service-panel="{{ $mirrorServiceKey }}" @if (! $loop->first) hidden @endif>
                <header class="unit-service-panel-header">
                  <div>
                    <h2>{{ $mirrorServiceName }}</h2>
                    <p>{{ $mirrorService?->category ?? 'Sin categoria' }} &middot; {{ $mirrorService?->specialty ?? 'Servicio general' }}</p>
                    <div class="unit-service-panel-badges">
                      <span class="unit-native-status">{{ $statusText($mirrorContract->status) }}</span>
                      <span>1 hospital habilitado</span>
                    </div>
                  </div>
                  <div class="unit-service-panel-actions">
                    <a href="{{ $mirrorOperationHref }}">Ver operacion</a>
                    <a href="{{ route('unit.services.report', ['contract' => $mirrorContract, 'unit' => $mirrorUnit->id]) }}">Descargar reporte</a>
                  </div>
                </header>

                <div class="unit-native-table-scroll">
                  @if ($mirrorIsConsultationService)
                    <table class="unit-native-table unit-consultation-table">
                      ...
                      <tbody>
                        @forelse ($mirrorUnitAppointments as $appointment)
                          @php
                          ...
                          <tr>
                            ...
                            <td><strong>{{ $appointment->medicalUnit?->name ?? $mirrorUnit->name }}</strong></td>
                            <td>{{ $mirrorConsultationRequestType($appointment) }}</td>
                            ...
                            <td><span class="unit-consultation-chip is-status-{{ $consultationStatusClass }}">{{ $consultationStatusLabel }}</span></td>
                  ...
                  @elseif ($mirrorIsNutritionService || $mirrorIsChemotherapyService)
                    @php
                      ...
                      <tbody>
                        @forelse ($mixtureRequests as $mixtureRequest)
                          ...
                          <tr>
                            ...
                            <td>{{ $mixId }}</td>
                            <td>{{ $requestNumber }}</td>
                            <td>{{ $mixtureRequest->medicalUnit?->name ?? $mirrorUnit->name }}</td>
                            ...
                            <td><span class="unit-consultation-chip is-status-{{ $mixtureStatusClass }}">{{ $mixtureStatusLabel }}</span></td>
                            <td>{{ $remission }}</td>
                            <td>{{ $lot }}</td>
                            <td><a class="unit-nutrition-view-link" href="{{ $mixtureRoute }}">Ver</a></td>
                          </tr>
                        @empty
                          <tr><td colspan="11" class="unit-native-empty">{{ $mixtureEmptyMessage }}</td></tr>
                        @endforelse
                      </tbody>
                    </table>
                  @else
                    <table class="unit-native-table unit-service-detail-table">
                      ...
                      <tbody>
                        <tr>
                          <td><strong>{{ $mirrorUnit->name }}</strong><small>{{ $mirrorUnit->type ?? $mirrorUnit->typology ?? 'Hospital General' }}</small></td>
                          <td>{{ $mirrorUnit->clues ?? $mirrorUnit->code ?? 'Sin CLUES' }}</td>
                          <td>{{ $mirrorUnitLocation ?: 'Sin ubicacion' }}</td>
                          <td>{{ $mirrorContract->starts_at?->format('d/m/Y') ?? 'Sin inicio' }} - {{ $mirrorContract->ends_at?->format('d/m/Y') ?? 'Sin vencimiento' }}</td>
                          <td>{{ $mirrorContract->contract_number ?? 'Sin contrato' }}</td>
                          <td><span class="unit-native-status">{{ $statusText($mirrorContract->status) }}</span></td>
                        </tr>
                      </tbody>
                    </table>
                  @endif
